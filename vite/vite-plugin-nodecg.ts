/**
 * License to https://github.com/RTAinJapan/rtainjapan-layouts.
 * Copyright to RTA in Japan Developers
 */
...
export default async ({
  ...
  const generateHtmlFiles = async () => {
    ...
    await Promise.all([
      (async (): Promise<void> => {
        if (graphicsInputs.length === 0) {
          return;
        }
        await fs.rm(graphicsOutdir, { recursive: true, force: true });
        await fs.mkdir(graphicsOutdir, { recursive: true });
      })(),
      (async (): Promise<void> => {
        if (dashboardInputs.length === 0) {
          return;
        }
        await fs.rm(dashboardOutdir, { recursive: true, force: true });
        await fs.mkdir(dashboardOutdir, { recursive: true });
      })(),
    ]);
    ...
    await Promise.all([
      ...graphicsInputs.map(async input => {
        await generateHtml(input, graphicsTemplateHtml, graphicsOutdir);
      }),
      ...dashboardInputs.map(async input => {
        await generateHtml(input, dashboardTemplateHtml, dashboardOutdir);
      }),
    ]);
  };
